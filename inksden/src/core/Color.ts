...
export default class Color {
    ...
    /**
     * 
     * @param color The color to be overlayed on top of the original color.
     * @returns `Color` class instance that represents the overlay of the provided color over the original one.
     */
    overlay(color: Color): Color {

        let red: number = color.red + this.red;
        let green: number = color.green + this.green;
        let blue: number = color.blue + this.blue;
        let opacity: number = color.opacity + this.opacity;
        red = (red > 255) ? 255 : red;
    ...
    /**
     * Static method to overlay two colors.
     * @param color1 The color on top of which the second color is to be overlayed.
     * @param color2 The color that will be overlayed on top of the first color.
     * @returns A `Color` class instance that is the overlayed Color of color2 over color1.
     */
    static overlayColors(color1: Color, color2: Color): Color {
        let red: number = color1.red + color2.red;
        let green: number = color1.green + color2.green;
        let blue: number = color1.blue + color2.blue;
        let opacity: number = color1.opacity + color2.opacity;
        red = (red > 255) ? 255 : red;
        green = (green > 255) ? 255 : green;
        blue = (blue > 255) ? 255 : blue;
        opacity = (opacity > 1) ? 1 : opacity;
    ...
    /**
     * Static method to overlay two colors by specifying the Hex values.
     * @param color1 The color on top of which the second color is to be overlayed.
     * @param color2 The color that will be overlayed on top of the first color.
     * @returns A `Color` class instance that is the overlayed Color of color2 over color1.
     */
     static overlayHexColors(color1: string, color2: string): Color {
        let color1Color = ColorHex(color1);
        let color2Color = ColorHex(color2);
        let red: number = color1Color.red + color2Color.red;
        let green: number = color1Color.green + color2Color.green;
        let blue: number = color1Color.blue + color2Color.blue;
        let opacity: number = color1Color.opacity + color2Color.opacity;
        red = (red > 255) ? 255 : red;
        green = (green > 255) ? 255 : green;
        blue = (blue > 255) ? 255 : blue;
        opacity = (opacity > 1) ? 1 : opacity;
